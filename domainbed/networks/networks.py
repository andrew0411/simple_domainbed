...
class SqueezeLastTwo(nn.Module):
    ...
    def init(self):
        super(SqueezeLastTwo, self).init()
...
class ResNet_mixstyle(nn.Module):
    def init(self, input_shape, hparams, network=None):
        super(ResNet_mixstyle, self).init()
        if hparams["resnet18"]:
            if network is None:
...
class ResNet(nn.Module):
    ...
    def init(self, input_shape, hparams):
